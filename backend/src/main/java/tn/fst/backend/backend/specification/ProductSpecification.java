...
public class ProductSpecification {
    ...
    public static Specification<Product> buildSpecification(ProductSearchRequest request) {
        return (root, query, criteriaBuilder) -> {
            ...
            if (request.getCategoryId() != null) {
                ...
                Predicate exactCategory = criteriaBuilder.equal(
                        categoryJoin.get("idCategory"), request.getCategoryId()
                );

                Predicate parentCategory = criteriaBuilder.equal(
                        categoryJoin.get("parentCategory").get("idCategory"),
                        request.getCategoryId()
                );

                predicates.add(criteriaBuilder.or(exactCategory, parentCategory));
            }
    ...
    /**
     * Search by keyword only (simpler version)
     */
