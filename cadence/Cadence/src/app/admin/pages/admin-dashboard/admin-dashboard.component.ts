...
import { DashboardService, KPIStats, SalesPerDay, ProductStat, CategoryOrders } from '../../services/dashboard.service';
...
export class AdminDashboardComponent implements OnInit {
  ...
  // Analytics Data
  kpiStats: KPIStats = { revenue: 0, orders: 0, clients: 0 };
  ...
  loadDashboard() {
    ...
    this.adminService.getFullDashboard().subscribe({
      next: (data) => {
        ...
        // Populate Charts
        // We always try to update, even if analytics is null (using stats fallbacks)
        ...
        if (stats.topCategories && stats.topCategories.length > 0) {
          this.productPieChartData.labels = stats.topCategories.map((c: any) => c.categoryName);
          this.productPieChartData.datasets[0].data = stats.topCategories.map((c: any) => Number(c.revenue || 0));
        }
        ...
        if (stats.ordersByStatus && Object.keys(stats.ordersByStatus).length > 0) {
          ...
          this.categoryBarChartData.labels = statusLabels;
          this.categoryBarChartData.datasets[0].data = statusValues as number[];
        }
  ...
  private updateProductChart(products: ProductStat[]): void {
    this.productPieChartData.labels = products.map(p => p.name);
    this.productPieChartData.datasets[0].data = products.map(p => p.value);
  }

  private updateCategoryChart(categories: CategoryOrders[]): void {
    this.categoryBarChartData.labels = categories.map(c => c.category);
    this.categoryBarChartData.datasets[0].data = categories.map(c => c.orders);
  }
  ...
  private buildKpiCards(stats: DashboardStats) {
    const symbol = stats.currency === 'USD' ? '$' : 'TND ';
    this.kpiCards = [
      {
  ...
  get formattedRevenue(): string {
    return `$${this.kpiStats.revenue.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`;
  }

  get formattedOrders(): string {
    return this.kpiStats.orders.toLocaleString();
  }

  get formattedClients(): string {
    return this.kpiStats.clients.toLocaleString();
  }
}
